...
public class AgentFrameworkService : IDisposable
{
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        // Some Foundry model deployments emit only response.output_text.done and no deltas.
        // Track delta output so the final event can be used as a fallback without duplicating text.
        bool emittedTextDelta = false;
                ...
                cancellationToken: cancellationToken))
        {
            ...
            {
                if (!emittedTextDelta && !string.IsNullOrEmpty(textDoneUpdate.Text))
                {
                    _logger.LogDebug("No text deltas received; emitting text from output_text.done");
                    yield return StreamChunk.Text(textDoneUpdate.Text);
                }
            }

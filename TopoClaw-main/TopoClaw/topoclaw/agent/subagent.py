...
from topoclaw.utils.helpers import build_assistant_message

if TYPE_CHECKING:
    from topoclaw.secure.toolcall_guard import ToolcallGuard
            ...
            while iteration < max_iterations:
                ...
                if response.has_tool_calls:
                    ...
                    for tool_call in response.tool_calls:
                        ...
                        if self.toolcall_guard is not None:
                            vr = self.toolcall_guard.validate_tool_call(
                                tool_call.name, tool_call.arguments
                            )
                            if not vr.allowed:
                                detail = vr.detail or vr.reason or "blocked"
                                logger.warning(
                                    "Subagent [{}] ToolcallGuard blocked {}: {}",
                                    task_id,
                                    tool_call.name,
                                    detail,
                                )
